    ...
    for (x = 0; x != width; ++x) {
      ...
      for (v = -yBound; v != yBound + adjustY; ++v) {   //-1, 0, 1
        for (u = -xBound; u != xBound + adjustX; ++u) { //-1, 0, 1
          ...
          mask[2] = 0xff;
          mask[3] = 0;
          initiator.write_to_socket(Gaussian_MM_BASE + Gaussianblur_R_ADDR, mask,
                                    data.uc, 4);
      ...
      int output_num=0;
      while(!done){
        initiator.read_from_socket(Gaussian_MM_BASE + Gaussianblur_CHECK_ADDR, mask, data.uc, 4);
        output_num = data.sint;
        if(output_num>0) done=true;
      }
      wait(10 * CLOCK_PERIOD, SC_NS);

      initiator.read_from_socket(Gaussian_MM_BASE + Gaussianblur_RESULT_ADDR, mask,
                                 data.uc, 4);
      total = data.sint;
